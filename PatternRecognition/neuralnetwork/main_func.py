...
layers_info = [LayerInfo(ActivationFunctions.SIGMOID, 64),
               LayerInfo(ActivationFunctions.TANH, 64),
               LayerInfo(ActivationFunctions.LINEAR, 1)]
NN = NeuralNetwork(layers_info, 1)
...
dataset.initialize(400)

x = []
y = []
for i in range(len(dataset)):
    x.append(np.array([dataset.x()[i]]))
...
for i in range(len(valid)):
    x_valid.append(np.array([valid.x()[i]]))
    y_valid.append(np.array([valid.y()[i]]))

x_valid = np.array(x)
y_valid = np.array(y)

NN.sample_fit_one_output_function(x_train, y_train, x_valid, y_valid, ErrorFunctions.MSE, h=0.005, epochs=500)
...
for i in range(len(test)):
    x_test.append(np.array([test.x()[i]]))
